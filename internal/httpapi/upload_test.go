...
func TestUploadLimitAndSafeName(t *testing.T) {
	cfg := config.Config{MaxUploadMB: 1, DataDir: t.TempDir(), SessionPolicy: config.SessionPolicyRejectSecond, UploadRatePerSec: 100, UploadRateBurst: 100, WSRatePerSec: 100, WSRateBurst: 100}
	logger := zap.NewNop()
	metrics := observability.NewMetrics(prometheus.NewRegistry())
	store, _ := storage.NewFrameStore(cfg.DataDir)
	...
	if w.Code != http.StatusBadRequest {
		t.Fatalf("expected 400 for large payload, got %d", w.Code)
	}
